...
# -----------------------------------------------------------
# 0. Auto-detectar Sprint 39 (cerrado) y Sprint 40 (activo)
# -----------------------------------------------------------
Write-Host "`n=== PASO 0: Detectando Sprints 39 y 40 en Jira ===" -ForegroundColor Yellow
...
$s40Sprint = $activeSprints.values | Sort-Object id -Descending | Select-Object -First 1
if (-not $s40Sprint) {
    ...
    try {
        ...
        $s40Sprint = $recentSprints.values | Where-Object { $_.name -match "40" } | Select-Object -First 1
    } catch {}
    if (-not $s40Sprint) {
        Write-Host "AVISO: No se encontro Sprint 40. Solo se procesara hasta Sprint 39." -ForegroundColor Yellow
        $SPRINT_ID_40 = $null
    } else {
        $SPRINT_ID_40 = $s40Sprint.id
        Write-Host "Sprint 40 encontrado: ID=$SPRINT_ID_40  Nombre=$($s40Sprint.name)" -ForegroundColor Green
    }
} else {
    $SPRINT_ID_40 = $s40Sprint.id
    Write-Host "Sprint activo (40) detectado: ID=$SPRINT_ID_40  Nombre=$($s40Sprint.name)" -ForegroundColor Green
}
...
if ($s39Closed) {
    $SPRINT_ID_39 = $s39Closed.id
    Write-Host "Sprint 39 (cerrado) confirmado: ID=$SPRINT_ID_39  Nombre=$($s39Closed.name)" -ForegroundColor Green
} else {
    Write-Host "ERROR: Sprint 39 no encontrado como cerrado en el board." -ForegroundColor Red
    exit 1
}
...
$keys40 = if ($s40issues.Count -gt 0) { $s40issues | Select-Object -ExpandProperty key } else { @() }
...
$allForChangelog = @{}
foreach ($t in ($s35issues + $s36issues + $s37issues + $s38issues + $s39issues + $s40issues)) {
...
# Sprint 39 (CERRADO): marcar Arrastrado los que continuaron a S40
$lineas39 = @()
...
# Sprint 40 (activo): estado real
$lineas40 = @()
foreach ($t in $s40issues) {
    $lineas40 += Build-TicketLine -t $t -sprintLabel "40"
}

$freshLines = $lineas35 + $lineas36 + $lineas37 + $lineas38 + $lineas39 + $lineas40
...
$sprintRange = if ($s40issues.Count -gt 0) { "Sprint 30 al 40" } else { "Sprint 30 al 39" }
...
Write-Host "  Historicos (S30-S34): $($historicalLines.Count)  |  S35: $($lineas35.Count)  |  S36: $($lineas36.Count)  |  S37: $($lineas37.Count)  |  S38: $($lineas38.Count)  |  S39: $($lineas39.Count)  |  S40: $($lineas40.Count)"
...
# -----------------------------------------------------------
# 6. KPIs Sprint 39 (cierre) y Sprint 40 (activo)
# -----------------------------------------------------------
Write-Host "`n=== KPIs Sprint 39 (cierre) ===" -ForegroundColor Yellow
...
$s39arr    = $s39issues | Where-Object { $keys40 -contains $_.key }
$s39sp     = 0; $s39fin | Where-Object { $_.fields.customfield_10016 } | ForEach-Object { $s39sp += [double]$_.fields.customfield_10016 }
$s39sp     = [Math]::Round($s39sp, 1)
$velS39    = if ($s39issues.Count -gt 0) { [Math]::Round($s39fin.Count * 100 / $s39issues.Count, 1) } else { 0 }
Write-Host "Finalizados en S39  : $($s39fin.Count) / $($s39issues.Count)"
Write-Host "Arrastrados a S40   : $($s39arr.Count)"
Write-Host "SP completados S39  : $s39sp"
Write-Host "Tasa completado S39 : $velS39 %"

if ($s40issues.Count -gt 0) {
    Write-Host "`n=== Sprint 40 (activo) ===" -ForegroundColor Yellow
    $s40fin = $s40issues | Where-Object {
        $_.fields.status.name -eq "Finalizada" -or
        $_.fields.status.name -eq "Done" -or
        $_.fields.status.name -eq "Finalizados"
    }
    $s40abiertos = $s40issues | Where-Object {
        $_.fields.status.name -ne "Finalizada" -and $_.fields.status.name -ne "Done"
    }
    Write-Host "Total tickets S40   : $($s40issues.Count)"
    Write-Host "Finalizados         : $($s40fin.Count)"
    Write-Host "Abiertos/En curso   : $($s40abiertos.Count)"
    $s40issues | Group-Object { $_.fields.status.name } | Select-Object Name, Count | Sort-Object Count -Descending | Format-Table -AutoSize
}
...
$allTicketsCombined = $s35issues + $s36issues + $s37issues + $s38issues + $s39issues + $s40issues | Group-Object key | ForEach-Object { $_.Group[0] }
...
$sprintActivoIssues = if ($s40issues.Count -gt 0) { $s40issues } else { $s39issues }
...
$sprintActivoNum  = if ($s40issues.Count -gt 0) { 40 } else { 39 }
$sprintCerradoNum = if ($s40issues.Count -gt 0) { 39 } else { 38 }
...
Write-Host ("  dashboard_data.js           " + $allLineas.Count + " entradas (S30-S34 historico + S35-S40 fresco)")
Write-Host ("  dashboard_changelog_data.js " + $changelogMap.Count + " tickets con changelog")
Write-Host "  kpis_resumen.json"
Write-Host ("`nSprint 39 cerrado : " + $velS39 + " pct completado")
if ($s40issues.Count -gt 0) {
    Write-Host ("Sprint 40 activo  : " + $s40issues.Count + " tickets, " + $s40abiertos.Count + " abiertos")
}
